...
class CgenSupport {
    ...
    private static int labelControl = 0;
    /**
    ...
    // prologo para inicializacion de objetos, stack
    public static void emitObjectInitPrologue(PrintStream str){
        emitAddi(SP, SP, -12, str);
        emitStore(FP, 3, SP, str);
        emitStore(SELF, 2, SP, str);
        emitStore(RA, 1, SP, str);
        emitAddi(FP, SP, 4, str);
        emitMove(SELF, ACC, str);
    }

    // regresar el stack
    public static void emitStackRestore(PrintStream str) {
    ...
    static void emitArith(Expression e1, Expression e2, String opcode, PrintStream str) {
        // generar codigo de e1
        e1.code(str);
        // cargar el valor de ACC + 12 y ponerlo en T1
        emitFetchInt(T1, ACC, str);
        // pushar T1 en el stack 
        emitPush(T1, str);
        CgenClassTable.codeGegerationClassTable.fp = CgenClassTable.codeGegerationClassTable.fp + 1;
        // generar codigo de e2
        e2.code(str);
        CgenClassTable.codeGegerationClassTable.fp = CgenClassTable.codeGegerationClassTable.fp - 1;
        // copia de object
        emitJal("Object.copy", str);  
        // cargar cargar el valor de ACC + 12 y ponerlo en T2
        emitFetchInt(T2, ACC, str);
        // obtener el valor de T1 y mover el sp
        emitAddi(SP, SP, 4, str);
        emitLoad(T1, 0, SP, str);

        // emitir instruccion
        str.println(opcode + T1 + " " + T1 + " " + T2);
        // guardar el resultado de T1 en ACC + 12
        emitStoreInt(T1, ACC, str);
    }
}
